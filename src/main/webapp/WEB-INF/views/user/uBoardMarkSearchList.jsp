					...
					<section class="contents">
						<div>
							<div class="title">
								...
								<div class="info_h1">
									<h1 style="font-size: 1em; font-weight: bold;">북마크 목록</h1>
								</div>
							</div>
							<div class="info_hr">
								<hr>
							...
							<div id="boardTable" class="board-list">
								...
								<div class="filter input-group mb-3">
									<select name="searchCondition"
										...
										<option value="count"
											<c:if test="${param.searchCondition == 'count' }">selected</c:if>>조회수</option>
										<option value="date"
...
<%-- 										<td style="text-align: left;">${bMList.fTitle}</td> --%>
										<c:set var="inputString" value="${bMList.fTitle}" /> <!-- sList.sTitle 값을 inputString 변수에 저장 -->
											<td style="text-align: left; padding-left: 25px;"> <!-- 왼쪽 정렬 스타일을 적용 -->
											    <c:choose>
											        <c:when test="${fn:length(inputString) > 19}"> <!-- 만약 문자열 길이가 5를 초과한다면 -->
											            <c:set var="truncatedString" value="${fn:substring(inputString, 0, 19)}..." /> <!-- 문자열을 자르고 "..."을 추가하여 truncatedString 변수에 저장 -->
										...
										<c:set var="formattedDate" value="${fn:replace(bMList.fCreate, '-', '.')}"/>
										<td>${formattedDate}</td>
						...
						<div aria-label="Page navigation example" class="page">
							...
							<c:url var="prevUrl" value="/user/searchBoardMark.pet">
							...
							<ul style="display: flex; justify-content: center;">
								...
								<c:forEach begin="${aInfo.startNavi }" end="${aInfo.endNavi }"
									var="p">
									<c:url var="pageUrl" value="/user/searchBoardMark.pet">
								...
								<c:if test="${aInfo.endNavi != aInfo.naviTotalCount }">
									<c:url var="nextUrl" value="/user/searchBoardMark.pet">
