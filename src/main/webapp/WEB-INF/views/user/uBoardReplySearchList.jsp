									...
									<c:otherwise>
										...
										<c:forEach items="${bRList }" var="bRList" varStatus="i">
											...
											<td style="text-align: left; padding-left: 25px;"> <!-- 왼쪽 정렬 스타일을 적용 -->
											    <c:choose>
											        <c:when test="${fn:length(inputString) > 11}"> <!-- 만약 문자열 길이가 5를 초과한다면 -->
											            <c:set var="truncatedString" value="${fn:substring(inputString, 0, 11)}..." /> <!-- 문자열을 자르고 "..."을 추가하여 truncatedString 변수에 저장 -->
...
<%-- 										<td>${bRList.fTitle}</td> --%>
											...
											<td style="text-align: left; padding-left: 25px;"> <!-- 왼쪽 정렬 스타일을 적용 -->
												...
												<td><fmt:formatDate value="${bRList.fRCreate}"
						...
						<div aria-label="Page navigation example" class="page">
							...
							<c:url var="prevUrl" value="/user/searchBoardReply.pet">
							...
							<ul style="display: flex; justify-content: center;">
								...
								<c:forEach begin="${aInfo.startNavi }" end="${aInfo.endNavi }"
									var="p">
									<c:url var="pageUrl" value="/user/searchBoardReply.pet">
								...
								<c:if test="${aInfo.endNavi != aInfo.naviTotalCount }">
									<c:url var="nextUrl" value="/user/searchBoardReply.pet">
